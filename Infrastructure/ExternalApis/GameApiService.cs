...
namespace VideoGameLibraryAndroid.Infrastructure.ExternalApis
{
    public class GameApiService : IGameApiService
    {
        ...
        // Una búsqueda por nombre es mucho más ambigua que por código de barras -- "Zelda" o
        // "Mario Kart" corresponden a bastantes juegos distintos (una entrada por plataforma en
        // IGDB/RAWG). 6 se quedaba corto para esos casos y descartaba resultados válidos antes
        // de que el usuario pudiera verlos en GameCandidatePickerPage (que sí soporta una lista
        // larga con scroll, a diferencia del antiguo DisplayActionSheet). 25 se queda por debajo
        // del page_size máximo de RAWG (40).
        private const int MaxNameSearchCandidates = 25;
        ...
        private async Task<List<Game>> SearchIgdbCandidatesAsync(string name, CancellationToken ct)
        {
            try
            {
                ...
                request.Headers.Add("Authorization", $"Bearer {token}");
                request.Content = new StringContent(
                    $"search \"{name}\"; fields name,cover.url,genres.name,platforms.name,first_release_date; limit {MaxNameSearchCandidates};");
        ...
        private async Task<List<Game>> SearchRawgCandidatesAsync(string name, CancellationToken ct)
        {
            try
            {
                ...
                var url = $"https://api.rawg.io/api/games?search={Uri.EscapeDataString(name)}&key={_rawgApiKey}&page_size={MaxNameSearchCandidates}";
                var response = await _http.GetStringAsync(url, ct);
                var json = JObject.Parse(response);
