...
namespace VideoGameLibraryAndroid.Tests
{
    ...
    // poder correr en paralelo sin interferirse; se borra en Dispose(). A diferencia de
    // VideoGameLibrary.Tests (escritorio), aquí no hay Migrate() -- este GameRepository no aplica
    // migraciones (ver comentario en GameRepository) -- así que el esquema se crea con
    // EnsureCreated() a partir del modelo de GameDbContext antes de construir el repositorio.
    public class GameRepositoryTests : IDisposable
    {
        ...
        {
            var dbName = $"videogamelibraryandroidtests_{Guid.NewGuid():N}";
