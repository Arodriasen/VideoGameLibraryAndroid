...
namespace VideoGameLibraryAndroid.Tests
{
    ...
    public class ImportServiceParseCsvTests : IDisposable
    {
        ...
        [Fact]
        public void ReadHeaders_y_GuessMapping_reconocen_columnas_de_un_csv_propio()
        {
            var path = WriteCsv("Código de Barras;Título;Plataforma\n123;Juego;PC\n");

            var headers = new ImportService().ReadHeaders(path);
            var mapping = ImportService.GuessMapping(headers);

            Assert.Equal(0, mapping["código de barras"]);
            Assert.Equal(1, mapping["título"]);
            Assert.Equal(2, mapping["plataforma"]);
        }
    }

    public class ImportServiceParseExcelTests : IDisposable
    {
        private readonly List<string> _tempFiles = new();

        private string WriteXlsx(string[] headers, params string?[][] rows)
        {
            ...
            using (var workbook = new ClosedXML.Excel.XLWorkbook())
            {
                ...
                for (int row = 0; row < rows.Length; row++)
                    for (int col = 0; col < rows[row].Length; col++)
                        sheet.Cell(row + 2, col + 1).Value = rows[row][col] ?? string.Empty;
        ...
        [Fact]
        public void Parsea_fila_basica_de_un_xlsx()
        {
            var path = WriteXlsx(
                new[] { "Título", "Plataforma", "Año" },
                new[] { "Super Mario Odyssey", "Nintendo Switch", "2017" });
        ...
        [Fact]
        public void Fila_sin_titulo_se_descarta_en_xlsx()
        {
            var path = WriteXlsx(
                new[] { "Título" },
                new[] { (string?)null },
